...
import { Stethoscope, ArrowRight, CheckCircle, Loader2 } from 'lucide-react';
...
export default function RegisterMedicalPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-white to-green-50 flex flex-col" dir="rtl">
      {/* Header */}
      ...
      <div className="flex-1 flex items-start justify-center px-4 py-10">
        <div className="w-full max-w-xl bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
          {/* Title */}
          <div className="bg-gradient-to-l from-cyan-700 to-primary px-8 py-6 text-white">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
                <Stethoscope className="w-5 h-5" />
              </div>
              <h2 className="text-xl font-black">تسجيل حالة مرضية</h2>
            </div>
            <p className="text-sm text-cyan-100 leading-relaxed">
              للحالات المرضية التي تحتاج دعم مادي للعلاج أو شراء الأدوية أو تكاليف العمليات.
            </p>
          </div>
            ...
            {done ? (
              <div className="text-center py-8 space-y-4">
                ...
                <p className="text-sm text-gray-500 leading-relaxed">
                  سيتم مراجعة حالتك الطبية من قِبل إدارة الجمعية والتواصل معك.
                </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-700">اسم المريض <span className="text-red-500">*</span></label>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input name="address" placeholder="القرية / الشارع" className="form-input-modern" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-700">التشخيص الطبي</label>
                    <input name="diagnosis" placeholder="مثال: قصور كلوي، سرطان..." className="form-input-modern" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-700">المستشفى / المركز الطبي</label>
                    <input name="hospital" placeholder="اسم المستشفى أو العيادة" className="form-input-modern" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-700">عدد أفراد الأسرة</label>
                    <input name="familySize" type="number" min={1} max={30} placeholder="مثال: 4" className="form-input-modern" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-700">تكلفة العلاج التقريبية (ج.م)</label>
                    <input name="monthlyIncome" type="number" min={0} placeholder="0" className="form-input-modern" />
                  </div>
                  <div className="space-y-1.5 sm:col-span-2">
                    <label className="text-sm font-bold text-gray-700">تفاصيل الاحتياج الطبي</label>
                    <input name="needDetails" placeholder="أدوية، عملية، جلسات غسيل..." className="form-input-modern" />
                  </div>
                  <div className="space-y-1.5 sm:col-span-2">
                    <label className="text-sm font-bold text-gray-700">ملاحظات إضافية</label>
                    <textarea name="notes" rows={3} placeholder="أي تفاصيل أخرى..." className="form-input-modern resize-none" />
                  </div>
                </div>
                ...
                >
                  {pending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Stethoscope className="w-4 h-4" />}
                  {pending ? 'جارٍ الإرسال...' : 'إرسال الطلب الطبي'}
                </button>
              </form>
            )}
          </div>
